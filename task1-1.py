...
def check_2(lst_obj):
    ...
    """
    for j in range(len(lst_obj)):          # О(n) - линейная
        if lst_obj[j] in lst_obj[j+1:]:    # О(n) - линейная
...
def check_3(lst_obj):
    ...
    """
    lst_copy = list(lst_obj)                 # O(len(...)) - линейная
    lst_copy.sort()                          # O(n log n) - линейно-логарифмическая
    for i in range(len(lst_obj) - 1):        # О(n) - линейная
